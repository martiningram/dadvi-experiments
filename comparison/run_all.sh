...
POTUS_COVERAGE_DIR='/media/martin/External Drive/projects/lrvb_paper/potus_coverage'

# for MODEL_NAME in potus; do
# 
# 	# python fit_raabbvi.py "$MODEl_NAME" "$TARGET_DIR"
# 	# python fit_dadvi.py "$MODEl_NAME" "$TARGET_DIR" 2 30
...
# For POTUS, just rerun 10 times for now
# TODO Warm starts?
for M in 8 16 32 64; do
	for rerun in `seq 1 10`; do
		echo "Rerun $rerun for M=$M"
		python fit_dadvi.py potus "$POTUS_COVERAGE_DIR/$M/$rerun" $rerun $M
	done
done
